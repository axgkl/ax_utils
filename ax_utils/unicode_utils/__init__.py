...
try:
    ...
    from ._convert_nested import decode_nested
except:
    pass
...
try:
    ...
    def as_basestring(ob): return '%s' % (ob,)
...
from .monkey_patches import set_defaultencoding
from .monkey_patches import json_loads_bytes
...
def ax_backslashreplace_backport(error):
    ...
    replaced = u''.join(u'\\x{:02x}'.format(as_int(c)) for c in bad_slice)
