...
def set_defaultencoding(encoding='utf-8', force=None):
    ...
    if not 'ascii' in cur_enc:
        if not force:
            ...
            # w/o probs. So:
            out(('NOT setting defaultencoding to %s - already changed to %s. '
                 'Consider the force flag.') % (encoding, _cur_enc))
            return
        else:
            out('Defaultencoding already changed to %s! Forcing to %s.' % (
                (_cur_enc, encoding)))
...
def json_loads_bytes():
    ...
    def patch_lib(lib):
        ...
        out(('%10s: Patched to return bytes at deserialization. '
             'Use %s.loads_decoded() to get unicode.') % ((lib.__name__,) * 2))
